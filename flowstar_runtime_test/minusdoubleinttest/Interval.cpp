...
Real& Real::operator = (const Real& r)
{
	...
	Interval Interval::pow(const int n) const
	{
		...
		if (lo <= 0) {			// 0 <= lo <= up signs flipped
			//setMode(FE_DOWNWARD);
			result.lo = -std::pow(lo, n); //number def positive which means stores as def negative or 0
			//setMode(FE_UPWARD);
			result.up = std::pow(up, n);
		}
		else if (up <= 0) {	// lo <= up <= 0
			//setMode(FE_DOWNWARD);
			result.lo = -std::pow(up, n); //number def positive which means stores as def negative or 0
			//setMode(FE_UPWARD);
	...
	pi_lo=-std::stod(str_pi_lo); //pi_lo is flipped because of rounding
	tmp_up=up/-pi_lo;
	tmp_up*=2;
	tmp_up=floor(tmp_up);
	//setMode(FE_DOWNWARD);
