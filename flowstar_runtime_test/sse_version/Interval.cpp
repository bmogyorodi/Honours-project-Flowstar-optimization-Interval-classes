...
Real& Real::operator = (const Real& r)
{
	...
	void Interval::remove_midpoint(Interval & M)
	{
		...
		interval=_mm_sub_pd(interval,M.interval); // sign change
		restoreMode();
	}
	...
	void Interval::remove_midpoint()
	{
		...
		//setMode(FE_DOWNWARD);	
		interval=_mm_sub_pd(interval,temp);
		restoreMode();
	}
	...
	inline double Interval::mulMin(const Interval& I) const {
		...
		__m128d matchprod=_mm_mul_pd(_mm_mul_pd(interval,I.interval),_mm_set1_pd(-1));
	...
	inline double Interval::mulMax(const Interval& I) const {
		...
		__m128d crossprod=_mm_mul_pd(_mm_mul_pd(interval,_mm_shuffle_pd(I.interval,I.interval,1)),_mm_set1_pd(-1));
	...
	pi_up+=interval[0];
	//setMode(FE_DOWNWARD);
	pi_down=-std::stod(str_pi_lo);
	pi_down/=2;
	pi_down+=interval[1];
	Interval shift=Interval(-pi_down,pi_up);
	return shift.sin();
	...
	void Interval::pow_assign(const int n)
	{
		...
		if (interval[1] <= 0) {		// 0 <= lo <= up flipped sign
			...
			//setMode(FE_DOWNWARD);
			double tmp = -std::pow(interval[0], n);
			//setMode(FE_UPWARD);
	...
	void Interval::toString(std::string& result) const
	{
		...
		//setMode(FE_DOWNWARD);
		setMode(FE_UPWARD);
		sprintf(strTemp, "%.10f", interval[1]);
		...
		//std::fesetround(FE_UPWARD);
		//setMode(FE_UPWARD);
		sprintf(strTemp, "%.10f", interval[0]);
